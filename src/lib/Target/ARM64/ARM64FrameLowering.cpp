...
void ARM64FrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  if (HasFP) {
    // Issue    sub fp, sp, Offset or
    //          mov fp,sp          when Offset is zero.
  ...
  // Note: Use emitFrameOffset() like above for FP if the FrameSetup flag is needed.
  //
  ...
  if (needsFrameMoves) {
    ...
    //     sp = 10030          ::  stp x28, x27, [sp, #-32]!
    //     sp = 10010          ::  >> final value <<
    //     sp = 10010          ::  stp fp,lr, [sp, #16]
    //     fp = 10020          ::  add fp, sp, #16
    //
