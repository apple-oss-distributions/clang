...
// Branch analysis.
bool
ARM64InstrInfo::AnalyzeBranch(MachineBasicBlock &MBB,MachineBasicBlock *&TBB,
...
void ARM64InstrInfo::instantiateCondBranch(MachineBasicBlock &MBB, DebugLoc DL,
...
unsigned
ARM64InstrInfo::InsertBranch(MachineBasicBlock &MBB, MachineBasicBlock *TBB,
                               MachineBasicBlock *FBB,
...
static unsigned canFoldIntoCSel(const MachineRegisterInfo &MRI,
                                unsigned VReg, unsigned *NewVReg = 0) {
...
bool
ARM64InstrInfo::canInsertSelect(const MachineBasicBlock &MBB,
                                const SmallVectorImpl<MachineOperand> &Cond,
...
void ARM64InstrInfo::insertSelect(MachineBasicBlock &MBB,
  ...
  ARM64CC::CondCode CC;
  switch (Cond.size()) {
  default: llvm_unreachable("Unknown condition opcode in Cond");
  case 1: // b.cc
  ...
  case 3: { // cbz/cbnz
    ...
    bool Is64Bit;
    switch (Cond[1].getImm()) {
    default: llvm_unreachable("Unknown branch opcode in Cond");
    case ARM64::CBZW:  Is64Bit = 0; CC = ARM64CC::EQ; break;
    case ARM64::CBZX:  Is64Bit = 1; CC = ARM64CC::EQ; break;
    case ARM64::CBNZW: Is64Bit = 0; CC = ARM64CC::NE; break;
    case ARM64::CBNZX: Is64Bit = 1; CC = ARM64CC::NE; break;
    }
  ...
  case 4: { // tbz/tbnz
    ...
    switch (Cond[1].getImm()) {
    default: llvm_unreachable("Unknown branch opcode in Cond");
    case ARM64::TBZ:  CC = ARM64CC::EQ; break;
    case ARM64::TBNZ: CC = ARM64CC::NE; break;
    }
  ...
  BuildMI(MBB, I, DL, get(Opc), DstReg)
    .addReg(TrueReg).addReg(FalseReg).addImm(CC);
}
...
static bool
UpdateOperandRegClass(MachineInstr *Instr) {
  ...
  for (unsigned OpIdx = 0, EndIdx = Instr->getNumOperands();
       OpIdx < EndIdx; ++OpIdx) {
...
optimizeCompareInstr(MachineInstr *CmpInstr, unsigned SrcReg, unsigned SrcReg2,
  ...
  // Replace SUBSWrr with SUBWrr if CPSR is not used.
  int Cmp_CPSR =
      CmpInstr->findRegisterDefOperandIdx(ARM64::CPSR, true);
  if (Cmp_CPSR != -1) {
    unsigned NewOpc;
  ...
  switch (MI->getOpcode()) {
  default: return false;
  case ARM64::ADDSWrr:  case ARM64::ADDSWri:
  case ARM64::ADDSXrr:  case ARM64::ADDSXri:
  case ARM64::SUBSWrr:  case ARM64::SUBSWri:
  case ARM64::SUBSXrr:  case ARM64::SUBSXri:
    break;
...
unsigned ARM64InstrInfo::isGPRZero(const MachineInstr *MI) const {
  switch (MI->getOpcode()) {
  default:
    break;
  case ARM64::MOVZWi:
...
// modifying bits.
unsigned ARM64InstrInfo::isGPRCopy(const MachineInstr *MI) const {
  switch (MI->getOpcode()) {
  default:
    break;
...
// Return true if this instruction simply renames a general register without
// modifying bits.
unsigned ARM64InstrInfo::isFPRCopy(const MachineInstr *MI) const {
  switch (MI->getOpcode()) {
  default:
    break;
  case TargetOpcode::COPY: {
  ...
  case ARM64::ORRv16i8:
    if (MI->getOperand(1).getReg() == MI->getOperand(2).getReg()) {
      assert(MI->getDesc().getNumOperands() == 3
             && MI->getOperand(0).isReg() && "invalid ORRv16i8 operands");
      return true;
    }
  }
  return false;
...
void ARM64InstrInfo::suppressLdStPair(MachineInstr *MI) const {
  ...
  assert(MOSuppressPair < (1 << MachineMemOperand::MOTargetNumBits) &&
         "Too many target MO flags");
  (*MI->memoperands_begin())->setFlags(
    MOSuppressPair << MachineMemOperand::MOTargetStartBit);
}

bool ARM64InstrInfo::
getLdStBaseRegImmOfs(MachineInstr *LdSt, unsigned &BaseReg, unsigned &Offset,
...
MachineInstr*
ARM64InstrInfo::emitFrameIndexDebugValue(MachineFunction &MF,
...
static const
MachineInstrBuilder &AddSubReg(const MachineInstrBuilder &MIB,
...
void ARM64InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (ARM64::GPR32spRegClass.contains(DestReg) &&
    ...
    if (DestReg == ARM64::WSP || SrcReg == ARM64::WSP) {
      ...
      BuildMI(MBB, I, DL, get(ARM64::MOVZWi), DestReg)
        .addImm(0)
  ...
  if (ARM64::GPR64spRegClass.contains(DestReg) &&
    ...
    if (DestReg == ARM64::SP || SrcReg == ARM64::SP) {
      ...
      BuildMI(MBB, I, DL, get(ARM64::MOVZXi), DestReg)
        .addImm(0)
        .addImm(ARM64_AM::getShifterImm(ARM64_AM::LSL, 0));
    } else {
  ...
  if (ARM64::FPR128RegClass.contains(DestReg) &&
    ...
    BuildMI(MBB, I, DL, get(ARM64::ORRv16i8), DestReg)
      .addReg(SrcReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }
  ...
  if (ARM64::FPR64RegClass.contains(DestReg) &&
    ...
    DestReg = RI.getMatchingSuperReg(DestReg, ARM64::dsub,
                                     &ARM64::FPR128RegClass);
    SrcReg = RI.getMatchingSuperReg(SrcReg, ARM64::dsub,
                                    &ARM64::FPR128RegClass);
    BuildMI(MBB, I, DL, get(ARM64::ORRv16i8), DestReg)
      .addReg(SrcReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  ...
  if (ARM64::FPR32RegClass.contains(DestReg) &&
    ...
    DestReg = RI.getMatchingSuperReg(DestReg, ARM64::ssub,
                                     &ARM64::FPR128RegClass);
    SrcReg = RI.getMatchingSuperReg(SrcReg, ARM64::ssub,
                                    &ARM64::FPR128RegClass);
    BuildMI(MBB, I, DL, get(ARM64::ORRv16i8), DestReg)
      .addReg(SrcReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  ...
  if (ARM64::FPR16RegClass.contains(DestReg) &&
    ...
    DestReg = RI.getMatchingSuperReg(DestReg, ARM64::hsub,
                                     &ARM64::FPR128RegClass);
    SrcReg = RI.getMatchingSuperReg(SrcReg, ARM64::hsub,
                                    &ARM64::FPR128RegClass);
    BuildMI(MBB, I, DL, get(ARM64::ORRv16i8), DestReg)
      .addReg(SrcReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  ...
  if (ARM64::FPR8RegClass.contains(DestReg) &&
    ...
    DestReg = RI.getMatchingSuperReg(DestReg, ARM64::bsub,
                                     &ARM64::FPR128RegClass);
    SrcReg = RI.getMatchingSuperReg(SrcReg, ARM64::bsub,
                                    &ARM64::FPR128RegClass);
    BuildMI(MBB, I, DL, get(ARM64::ORRv16i8), DestReg)
      .addReg(SrcReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
...
void llvm::emitFrameOffset(MachineBasicBlock &MBB,
  ...
  while (((unsigned)Offset) >= (1 << ShiftSize)) {
    ...
    BuildMI(MBB, MBBI, DL, TII->get(Opc), DestReg)
      .addReg(SrcReg).addImm(ThisVal >> ShiftSize)
...
MachineInstr*
ARM64InstrInfo::foldMemoryOperandImpl(MachineFunction &MF,
                                      MachineInstr* MI,
...
int llvm::isARM64FrameOffsetLegal(const MachineInstr &MI, int &Offset,
  ...
  if (EmittableOffset) *EmittableOffset = 0;
  if (OutUseUnscaledOp) *OutUseUnscaledOp = false;
  if (OutUnscaledOp) *OutUnscaledOp = 0;
  switch (MI.getOpcode()) {
  default: assert(0 && "unhandled opcode in rewriteARM64FrameIndex");
  ...
  case ARM64::PRFMui:   Scale = 8; UnscaledOp = ARM64::PRFUMi; break;
  case ARM64::LDRXui:   Scale = 8; UnscaledOp = ARM64::LDURXi; break;
  case ARM64::LDRWui:   Scale = 4; UnscaledOp = ARM64::LDURWi; break;
  case ARM64::LDRBui:   Scale = 1; UnscaledOp = ARM64::LDURBi; break;
  case ARM64::LDRHui:   Scale = 2; UnscaledOp = ARM64::LDURHi; break;
  case ARM64::LDRSui:   Scale = 4; UnscaledOp = ARM64::LDURSi; break;
  case ARM64::LDRDui:   Scale = 8; UnscaledOp = ARM64::LDURDi; break;
  case ARM64::LDRQui:   Scale = 16; UnscaledOp = ARM64::LDURQi; break;
  case ARM64::LDRBBui:  Scale = 1; UnscaledOp = ARM64::LDURBBi; break;
  case ARM64::LDRHHui:  Scale = 2; UnscaledOp = ARM64::LDURHHi; break;
  case ARM64::LDRSBXui: Scale = 1; UnscaledOp = ARM64::LDURSBXi; break;
  case ARM64::LDRSBWui: Scale = 1; UnscaledOp = ARM64::LDURSBWi; break;
  case ARM64::LDRSHXui: Scale = 2; UnscaledOp = ARM64::LDURSHXi; break;
  case ARM64::LDRSHWui: Scale = 2; UnscaledOp = ARM64::LDURSHWi; break;
  case ARM64::LDRSWui:  Scale = 4; UnscaledOp = ARM64::LDURSWi; break;

  case ARM64::STRXui:   Scale = 8; UnscaledOp = ARM64::STURXi; break;
  case ARM64::STRWui:   Scale = 4; UnscaledOp = ARM64::STURWi; break;
  case ARM64::STRBui:   Scale = 1; UnscaledOp = ARM64::STURBi; break;
  case ARM64::STRHui:   Scale = 2; UnscaledOp = ARM64::STURHi; break;
  case ARM64::STRSui:   Scale = 4; UnscaledOp = ARM64::STURSi; break;
  case ARM64::STRDui:   Scale = 8; UnscaledOp = ARM64::STURDi; break;
  case ARM64::STRQui:   Scale = 16; UnscaledOp = ARM64::STURQi; break;
  case ARM64::STRBBui:  Scale = 1; UnscaledOp = ARM64::STURBBi; break;
  case ARM64::STRHHui:  Scale = 2; UnscaledOp = ARM64::STURHHi; break;
  ...
  if (Offset >= MinOff && Offset <= MaxOff) {
    if (EmittableOffset) *EmittableOffset = Offset;
    Offset = 0;
  } else {
    int NewOff = Offset < 0 ? MinOff : MaxOff;
    if (EmittableOffset) *EmittableOffset = NewOff;
    Offset = (Offset - NewOff) * Scale;
  }
  if (OutUseUnscaledOp) *OutUseUnscaledOp = useUnscaledOp;
  if (OutUnscaledOp) *OutUnscaledOp = UnscaledOp;
...
bool llvm::rewriteARM64FrameIndex(MachineInstr &MI, unsigned FrameRegIdx,
  ...
  unsigned UnscaledOp;
  bool UseUnscaledOp;
  int Status = isARM64FrameOffsetLegal(MI, Offset, &UseUnscaledOp,
