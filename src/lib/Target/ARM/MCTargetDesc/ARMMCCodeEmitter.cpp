...
public:
  ...
  /// getVSELcondValue - Return vsel encoding of the 'cc' bits.
  unsigned getVSELcondValue(const MCInst &MI, unsigned Op,
                           SmallVectorImpl<MCFixup> &Fixups) const {
    // The operand is the top two bits of the normal cond value.
    unsigned cond = MI.getOperand(Op).getImm();
    switch (cond) {
      default: assert(0 && "not a valid vsel condition!"); break;
      case ARMCC::GE: case ARMCC::GT: case ARMCC::EQ: case ARMCC::VS: break;
    }
    return (cond >> 2) & 0x3;
  }
