...
class ARMOperand : public MCParsedAsmOperand {
  ...
  struct MemoryOp {
    ...
    SMLoc AlignmentLoc;       // for error reporting if needed.
...
public:
  ...
  static ARMOperand *CreateMem(unsigned BaseRegNum,
    ...
    Op->Memory.AlignmentLoc = AlignmentLoc;
...
getMnemonicAcceptInfo(StringRef Mnemonic, StringRef FullInst,
  ...
  if (Mnemonic == "bkpt" || Mnemonic == "cbnz" || Mnemonic == "setend" ||
      ...
      Mnemonic.startswith("cps") || Mnemonic.startswith("crc32") ||
  ...
  if (hasV8Ops()) {
    if ((Mnemonic == "vmaxnm") || (Mnemonic == "vminnm"))
      CanAcceptPredicationCode = false;
    if ((Mnemonic.size() == 6) && Mnemonic.startswith("vrint"))
      if (Mnemonic.endswith("a") || Mnemonic.endswith("n") ||
          Mnemonic.endswith("p") || Mnemonic.endswith("m"))
        CanAcceptPredicationCode = false;
    if ((Mnemonic.size() == 5) && Mnemonic.startswith("vcvt"))
      if (Mnemonic.endswith("a") || Mnemonic.endswith("n") ||
          Mnemonic.endswith("p") || Mnemonic.endswith("m"))
        CanAcceptPredicationCode = false;
  }
