...
void ARMFrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  if (unsigned NumAlignedDPRCS2Regs = AFI->getNumAlignedDPRCS2Regs()) {
    // The realignment shenanigans start after all other pushes and the variadic
    // compensation:
    int PreOffset = GPRCS1Size + GPRCS2Size + DPRCSSize + ArgRegsSaveSize;

    // And their first step is essentially "sub sp, 8*NumRegs".
    PreOffset += 8 * NumAlignedDPRCS2Regs;

    // Then align the stack to the required amount:
    int Align = MF.getFrameInfo()->getMaxAlignment();

    // So, the final address of d[8+i] is: "(CFA - PreOffset) & -Align + 8*i"
