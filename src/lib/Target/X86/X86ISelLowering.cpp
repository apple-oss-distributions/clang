...
void X86TargetLowering::resetOperationActions() {
  ...
  setOperationAction(ISD::FROUND, MVT::f80, Expand);
...
X86TargetLowering::LowerBUILD_VECTORvXi1(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (In.getOpcode() == ISD::SETCC) {
    ...
    EFLAGS = EmitCmp(dl, Op0, Op1, X86CCVal, DAG);
    ...
    EFLAGS = EmitTest(dl, Bit1, X86::COND_NE, DAG);
...
SDValue X86TargetLowering::LowerBRCOND(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (addTest) {
    ...
    CC = DAG.getConstant(X86Cond, MVT::i8);
    Cond = EmitTest(dl, Cond, X86Cond, DAG);
  }
  Cond = ConvertCmpIfNecessary(Cond, DAG);
