...
MCDataFragment *MCObjectStreamer::getOrCreateDataFragment() {
  ...
  if (!F || (Assembler->isBundlingEnabled() && F->hasInstructions()) ||
      F->getContents().size() > (33*1024*1024 - 64)) {
    F = new MCDataFragment();
    insert(F);
  }
  return F;
...
void MCObjectStreamer::EmitDwarfAdvanceLineAddr(int64_t LineDelta,
  ...
  if (!LastLabel) {
    emitDwarfSetLineAddr(*this, Assembler->getDWARFLinetableParameters(),
                         LineDelta, Label, PointerSize);
    return;
  }
  const MCExpr *AddrDelta = buildSymbolDiff(*this, Label, LastLabel);
  int64_t Res;
  if (AddrDelta->EvaluateAsAbsolute(Res, getAssembler())) {
    MCDwarfLineAddr::Emit(this, Assembler->getDWARFLinetableParameters(),
                          LineDelta, Res);
    return;
  }
...
void MCObjectStreamer::FinishImpl() {
  ...
  MCDwarfLineTable::Emit(this, getAssembler().getDWARFLinetableParameters());
