...
static Value *MaybeSimplify(Value *V) {
  if (Instruction *I = dyn_cast<Instruction>(V))
    if (Value *Simplified = SimplifyInstruction(
            I, I->getParent()->getParent()->getParent()->getDataLayout()))
      return Simplified;
  return V;
}
...
InductiveRangeCheck::computeSafeIterationSpace(ScalarEvolution &SE,
  ...
  const DataLayout &DL =
      Branch->getParent()->getParent()->getParent()->getDataLayout();
  Value *OffsetV = SCEVExpander(SE, DL, "safe.itr.space").expandCodeFor(
