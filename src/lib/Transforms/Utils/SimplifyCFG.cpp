...
static bool SwitchToLookupTable(SwitchInst *SI,
  ...
  // When we have switches where the range of values representable by
  // the data type being switched on is identical to the range of
  // values represented by explicit cases, we attempt to generate a
  // lookup without any guard over the range. As part of this, we
  // remove the switch's block as a predecessor of the destination
  // block for the default case (guarded farther below by
  // GeneratingCoveredLookupTable).
  //
  // If there are elements of the range without explicit cases, we
  // attempt to insert a guard based on a mask (NeedMask). In this
  // case, we attempt to insert a new predecessor to a new block
  // (MaskBB below), transfering incoming values from the switch's
  // block. This asserts/crashes if GeneratingCoveredLookupTable is
  // also true, since we've already removed the switch's block as a
  // predecessor.
  //
  // As a very conservative fix we'll just bail out instead, meaning
  // that switches that have explicit cases with a range that is the
  // same as the range representable by the type being switched on,
  // but that are missing explicit cases for all values in the range,
  // will not get turned into lookup tables.
  if (GeneratingCoveredLookupTable && NeedMask)
    return false;
