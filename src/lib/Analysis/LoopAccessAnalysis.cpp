...
bool LoopAccessInfo::RuntimePointerCheck::needsChecking(
  ...
  if (PtrPartition && (*PtrPartition)[I] != -1 &&
    ...
    // If a store forwards to a load in a partition, add memchecks between this
    // load and all the other stores in the loop.  This allows elimination of
    // the load.
    unsigned ST = I, LD = J;
    if (!IsWritePtr[ST])
      std::swap(ST, LD);

    if (PtrsInStoreToLoadForwarding && !IsWritePtr[LD] &&
        PtrsInStoreToLoadForwarding->count(Pointers[LD]) &&
        !PtrsInStoreToLoadForwarding->count(Pointers[ST]))
      return true;
...
void LoopAccessInfo::RuntimePointerCheck::print(
    raw_ostream &OS, unsigned Depth, const SmallVectorImpl<int> *PtrPartition,
    const SmallSet<Value *, 8> *PtrsInStoreToLoadForwarding) const {
  ...
  for (unsigned I = 0; I < NumPointers; ++I)
    for (unsigned J = I + 1; J < NumPointers; ++J)
      if (needsChecking(I, J, PtrPartition, PtrsInStoreToLoadForwarding)) {
        OS.indent(Depth) << N++ << ":\n";
...
std::pair<Instruction *, Instruction *> LoopAccessInfo::addRuntimeCheck(
    Instruction *Loc, const SmallVectorImpl<int> *PtrPartition,
    const SmallSet<Value *, 8> *PtrsInStoreToLoadForwarding) const {
  if (!PtrRtCheck.Need)
