...
void CGDebugInfo::EmitDeclareOfBlockDeclRefVariable(const VarDecl *VD,
  ...
  llvm::Instruction *Call = InsertPoint ?
      DBuilder.insertDeclare(Storage, D, InsertPoint)
    : DBuilder.insertDeclare(Storage, D, Builder.GetInsertBlock());
  Call->setDebugLoc(
...
void CGDebugInfo::EmitImportDecl(const ImportDecl &ID) {
  DBuilder.createImportedDeclaration(
    getCurrentContextDescriptor(cast<Decl>(ID.getDeclContext())),
                                getOrCreateModule(ID.getImportedModule()),
...
llvm::DIModule
CGDebugInfo::getOrCreateModule(const Module *M) {
  auto I = ModuleCache.find(M);
  if (I != ModuleCache.end())
    return llvm::DIModule(cast<llvm::MDNode>(I->second));

  llvm::DIDescriptor Scope(
    M->isSubModule() ? getOrCreateModule(M->getTopLevelModule())
                     : TheCU);
  llvm::DIFile FileD =
    DBuilder.createFile(M->getASTFile()->getName(),
                        M->getASTFile()->getDir()->getName());

  llvm::DIModule Mod = DBuilder.createModule(Scope, M->getFullModuleName(),
                                             FileD, 1);
  ModuleCache.insert(std::make_pair(M, llvm::WeakVH(Mod)));
  return Mod;
}
