...
static bool isIntroducingInitializers(const ObjCInterfaceDecl *D) {
  for (ObjCInterfaceDecl::instmeth_iterator
         I = D->instmeth_begin(), E = D->instmeth_end(); I != E; ++I) {
    const auto *MD = *I;
    if (MD->getMethodFamily() == OMF_init && !MD->isOverriding())
      return true;
  }
  for (ObjCInterfaceDecl::visible_extensions_iterator
         ExtI = D->visible_extensions_begin(), ExtE = D->visible_extensions_end(); ExtI != ExtE; ++ExtI) {
    const auto *Ext = *ExtI;
    for (ObjCInterfaceDecl::instmeth_iterator
           I = Ext->instmeth_begin(), E = Ext->instmeth_end(); I != E; ++I) {
      const auto *MD = *I;
      if (MD->getMethodFamily() == OMF_init && !MD->isOverriding())
        return true;
    }
...
void ObjCInterfaceDecl::getDesignatedInitializers(
  ...
  for (ObjCInterfaceDecl::visible_extensions_iterator
         ExtI = IFace->visible_extensions_begin(), ExtE = IFace->visible_extensions_end(); ExtI != ExtE; ++ExtI) {
    const auto *Ext = *ExtI;
    for (instmeth_iterator I = Ext->instmeth_begin(),
                           E = Ext->instmeth_end(); I != E; ++I) {
      const ObjCMethodDecl *MD = *I;
      if (MD->isThisDeclarationADesignatedInitializer())
        Methods.push_back(MD);
    }
