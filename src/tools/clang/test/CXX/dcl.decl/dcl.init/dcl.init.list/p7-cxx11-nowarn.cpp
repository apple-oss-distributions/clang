// RUN: %clang_cc1 -fsyntax-only -std=c++11 -triple x86_64-apple-macosx10.6.7 -Wno-error=c++11-narrowing -verify %s
...
void float_to_int() {
  Agg<char> a1 = {1.0F};  // expected-warning {{ cannot be narrowed }} expected-note {{override}}
...
void shrink_float() {
  ...
  Agg<float> f2 = {d};  // expected-warning {{ cannot be narrowed }} expected-note {{override}}
  ...
  Agg<float> f8 = {1E50};  // expected-warning {{ cannot be narrowed }} expected-note {{override}}
