...
public:
  ...
  /// \brief Add a name accelerator entry for \a Die with \a Name.
  ...
  struct AccelInfo {
    DwarfStringPoolEntryRef Name; ///< Name of the entry.
    const DIE *Die;               ///< DIE this entry describes.
    uint32_t QualifiedNameHash;   ///< Hash of the fully qualified name
    bool SkipPubSection; ///< Emit this entry only in the apple_* sections.
    bool ObjcClassImplementation; ///< Is this an ObjC class implem?
...
private:
  ...
  /// Emit B&I warnings as Dwarf compile units.
  ...
  private:
    ...
    struct AttributesInfo {
      DwarfStringPoolEntryRef Name, MangledName, NameWithoutTemplate; ///< Names
      ...
      bool HasRanges;     ///< Does the DIE have a ranges attribute?
...
static const DWARFDebugInfoEntryMinimal *resolveDIEReference(
  ...
  // FIXME: We will drop this attribute when we clone.
