...
void LogFullErrorReport(const char *buffer) {
  ...
  // However, CRAppendCrashLogMessage does not work on iOS, so use an Apple
  // internal API to make sure get get proper CrashLog updates on the device.
#if HAVE_CRASHREPORTERCLIENT_H && SANITIZER_IOS && !SANITIZER_IOSSIM
