...
static void POSIXParseSymbolizedStackFrames(const char *str,
                                            SymbolizedStack *res) {
  uptr orig_addr = res->info.address;
  const char *orig_module_name = res->info.module;
  uptr orig_module_offset = res->info.module_offset;
  ...
  while (true) {
    ...
    if (top_frame) {
      ...
      cur = SymbolizedStack::New(orig_addr);
      cur->info.FillAddressAndModuleInfo(orig_addr, orig_module_name,
                                         orig_module_offset);
      last->next = cur;
      last = cur;
    }
...
class SymbolizerProcess : public ExternalSymbolizerInterface {
 public:
  ...
  void ParseSymbolizedStackFrames(const char *str, SymbolizedStack *res) {
    POSIXParseSymbolizedStackFrames(str, res);
  }
...
class Addr2LinePool : public ExternalSymbolizerInterface {
 public:
  ...
  char *SendCommand(uptr addr, bool is_data, const char *module_name,
    ...
    return addr2line->SendCommand(addr, is_data, module_name, module_offset);
  }
...
class POSIXSymbolizer : public Symbolizer {
 public:
  ...
  SymbolizedStack *SymbolizePC(uptr addr) override {
    ...
    const char *str = SendCommand(addr, false, module_name, module_offset);
    if (str == 0) {
      // Symbolizer was not initialized or failed.
      return res;
    }

    ParseSymbolizedStackFrames(str, res);
  ...
  bool SymbolizeData(uptr addr, DataInfo *info) override {
    ...
    const char *str = SendCommand(addr, true, module_name, module_offset);
    if (str == 0)
      return true;
    str = ExtractToken(str, "\n", &info->name);
 ...
 private:
  char *SendCommand(uptr addr, bool is_data, const char *module_name,
    ...
    if (internal_symbolizer_) {
      SymbolizerScope sym_scope(this);
      return internal_symbolizer_->SendCommand(addr, is_data, module_name,
                                               module_offset);
    }
    // Otherwise, fall back to external symbolizer.
    if (external_symbolizer_) {
      SymbolizerScope sym_scope(this);
      return external_symbolizer_->SendCommand(addr, is_data, module_name,
                                               module_offset);
    }
    return 0;
  }
  ...
  void ParseSymbolizedStackFrames(const char *str, SymbolizedStack *res) {
    ...
    if (internal_symbolizer_) {
      SymbolizerScope sym_scope(this);
      internal_symbolizer_->ParseSymbolizedStackFrames(str, res);
      return;
    }
    // Otherwise, fall back to external symbolizer.
    if (external_symbolizer_) {
      SymbolizerScope sym_scope(this);
      external_symbolizer_->ParseSymbolizedStackFrames(str, res);
      return;
    }
  }
...
#if SANITIZER_MAC
        if (!path_to_external)
          path_to_external = FindPathToBinary("atos");
        if (!path_to_external ||
            internal_strcmp(StripModuleName(path_to_external),
                            "llvm-symbolizer")) {
          external_symbolizer =
              new(symbolizer_allocator_) AtosSymbolizer(path_to_external);
        }
#endif  // SANITIZER_MAC

        if (!path_to_external)
          path_to_external = FindPathToBinary("llvm-symbolizer");
        if (path_to_external && !external_symbolizer) {
