...
public:
  ...
  struct RuntimePointerCheck {
    ...
    ///
    /// If \p PtrsInStoreToLoadForwarding is set, it contains the load and store
    /// pointers such that the stores forward to these load.  Also add memchecks
    /// for the loads from this set against all the *other* stores *within* the
    /// same partition.
    bool needsChecking(
        unsigned I, unsigned J, const SmallVectorImpl<int> *PtrPartition,
        const SmallSet<Value *, 8> *PtrsInStoreToLoadForwarding) const;

    /// \brief Return true if any pointer requires run-time checking according
    /// to needsChecking.
    ///
    /// See the explanation for the parameters under needsChecking.
    bool needsAnyChecking(
  ...
  /// for the loads from this set against all the *other* stores *within* the
  /// same partition.
  std::pair<Instruction *, Instruction *> addRuntimeCheck(
      Instruction *Loc, const SmallVectorImpl<int> *PtrPartition = nullptr,
      const SmallSet<Value *, 8> *PtrsInStoreToLoadForwarding = nullptr) const;
